...
function checkProfileChanges() {
    ...
    const changed = (fullName !== originalValues.full_name || username !== originalValues.username || email !== originalValues.email);
...
function validatePasswordField(fieldId) {
    if (fieldId === 'new_password') {
        ...
        if (val.length > 0 && val.length < 6) { showFieldError('new_password', 'Password must be at least 6 characters'); }
        else if (val.length >= 6) { markFieldValid('new_password'); }
    ...
    if (fieldId === 'confirm_password') {
        const newPw = document.getElementById('new_password').value;
        const confirm = document.getElementById('confirm_password').value;
        if (confirm.length > 0 && confirm !== newPw) { showFieldError('confirm_password', 'Passwords do not match'); }
        else if (confirm.length > 0 && confirm === newPw) { markFieldValid('confirm_password'); }
...
document.getElementById('passwordForm').addEventListener('submit', function(e) {
    ...
    let hasError = false;
    if (newPassword.length < 6) { showFieldError('new_password', 'Password must be at least 6 characters'); hasError = true; }
    if (newPassword !== confirmPassword) { showFieldError('confirm_password', 'Passwords do not match'); hasError = true; }
    if (hasError) { e.preventDefault(); return false; }
});
